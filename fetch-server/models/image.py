...
class WildlifeImage(db.Model):
    __tablename__ = 'wildlifeimage'
    id       = db.Column(db.Integer, primary_key=True)
    path     = db.Column(db.Unicode(255))
    wildlife_id = db.Column(db.Integer, db.ForeignKey('wildlife.id'))
    ...
    def get_fullpath(self):
        return op.join(app.config['FS_WILDLIFE_IMAGES_ROOT'], self.path)

class WildlifeImageAdmin(ModelView):
    column_list = ('path', 'image')
    ...
    def _list_thumbnail(view, context, model, value):
        ...
        return Markup('<img src="%s" style="max-height:300px;max-width:300px;height:auto;width:auto;">' % url_for('download_wildlife_image', image_id=model.id))
    ...
    # In this case, Flask-Admin won't attempt to merge various parameters for the field.
    form_extra_fields = {
        'path': form.ImageUploadField('WildlifeImage',
                                      allow_overwrite=False,
                                      base_path=app.config['FS_WILDLIFE_IMAGES_ROOT'])
    }

    def is_accessible(self):
        return current_user.has_role('superuser')

@listens_for(WildlifeImage, 'after_delete')
def del_image(mapper, connection, target):
    # Delete image
    try:
        os.remove(target.get_fullpath())
